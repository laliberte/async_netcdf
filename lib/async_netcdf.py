...
def end_parallel_instance(self,out,line):
    if line.lstrip()[14:].rstrip('\n').strip() != self.parallel_dimension:
        ...
        if self.stride==0:
            ...
            out.writei('STRIDE='+str(self.stride)+'\n')
        ...
        if self.debug:
            out.writei('parallel -j1 --tty "bash ${AN_ROOT_FILE}.'+self.parallel_dimension+'_function'+str(self.parallel_dimension_number)+'.sh {}"\n')
        else:
...
def main():
    ...
    setup_group.add_argument("--debug",dest="debug",
                      ...
                      help="Output shown in terminal.")
    setup_group.add_argument("--stride",dest="stride",
                      type=int,default=0,
                      help="The number of indices to process per processor.")
    setup_group.add_argument("--num_procs","-j",dest="num_procs",
                      default=1,
                      help="Number of simulataneous processors")
